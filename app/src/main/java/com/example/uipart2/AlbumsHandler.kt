...
class AlbumsHandler(var context: Context):SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    companion object{
...
//        TODO("Not yet implemented")
        val query = "CREATE TABLE "+ TABLE_NAME +" ("+ COL_ID +" INTEGER PRIMARY KEY, "+ COL_TITLE+" TEXT, "+ COL_RELEASE_DATE+" DECIMAL(10, 2)) "
        db?.execSQL(query)
    }
...
//        TODO("Not yet implemented")
    ...
    fun create(album: Album): Boolean{
        ...
        val convertedDate = String.format("%.2f", album.releaseDate).toDouble()
        contentValues.put(COL_RELEASE_DATE, convertedDate)
    ...
    fun read(): MutableList<Album>{
        ...
        if(cursor.moveToFirst()){
            do{
                ...
                releaseDate = cursor.getDouble(cursor.getColumnIndex(COL_ID))
    ...
    fun readOne(album_id: Int): Album {
        ...
        var id: Int
        var title: String
        var releasedDate: Double
        if(cursor.moveToFirst()){
            id = cursor.getInt(cursor.getColumnIndex(COL_ID))
            title = cursor.getString(cursor.getColumnIndex(COL_TITLE))
            releasedDate = cursor.getDouble(cursor.getColumnIndex(COL_RELEASE_DATE))
            album = Album(id, title, releasedDate)
        }
        return album
    }
